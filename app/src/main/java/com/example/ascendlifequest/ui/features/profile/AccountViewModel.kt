...
class AccountViewModel(
        private val authRepository: AuthRepository =
                com.example.ascendlifequest.data.auth.AuthRepositoryImpl(
                        com.example.ascendlifequest.data.remote.AuthService()
                )
) : ViewModel() {
    ...
    fun updateEmail(newEmail: String) {
        ...
        // Keep legacy Firebase behaviour because repository currently does not expose
            ...
            try {
                user.verifyBeforeUpdateEmail(newEmail)
                        .addOnSuccessListener {
                            val currentEmail =
                                    authRepository.getCurrentUser()?.email ?: lastKnownEmail
                            _uiState.value =
                                    AccountUiState.Success(
                                            "Un e-mail de vérification a été envoyé à $newEmail.\n\n" +
                                                    "IMPORTANT : Vous allez être déconnecté. Vérifiez votre boîte mail ($newEmail), " +
                ...
                _uiState.value =
                        AccountUiState.Error(
                                e.localizedMessage ?: "Erreur lors de la mise à jour de l'email",
            ...
            try {
                user.updatePassword(newPassword)
                            ...
                            _uiState.value =
                                    AccountUiState.Success(
                                            "Mot de passe mis à jour avec succès.\n\n" +
                                                    "IMPORTANT : Vous allez être déconnecté pour des raisons de sécurité. " +
                        ...
                        .addOnFailureListener { ex ->
                            ...
                            if (ex is FirebaseAuthRecentLoginRequiredException) {
                                _uiState.value =
                                        AccountUiState.ReauthRequired("password", newPassword)
                            } else {
                                _uiState.value =
                                        AccountUiState.Error(
                                                ex.message
    ...
    suspend fun reauthenticate(password: String): Result<Unit> =
            suspendCancellableCoroutine { cont ->
                ...
                if (email == null) {
                    cont.resume(
                            Result.failure(
                                    Exception("Email introuvable pour la ré-authentification")
                            )
                    )
                    return@suspendCancellableCoroutine
                }
